# CrowdRec-RL：众包任务推荐的强化学习项目

## 0. 快速开始

```bash
# 环境
conda activate RL          # Python 3.10, 已安装 pandas/numpy/sklearn/torch/matplotlib
...
- 154 个 worker 的 quality=-1，用**中位数 78** 填充，归一化到 [0,1]
- 过滤 status 无效的项目（draft/cancelled 等）
- 移除 entry_created_at 晚于项目 deadline 的异常记录
- **扩展 worker_df**：entry 中出现的 11,865 个 worker，原始 csv 仅 1,807 个，自动补全新 worker 行
...
### 3.3.5 Worker Quality 预测 (`src/quality_predictor.py`)  ★ 亮点

entry 中有 11,865 个 worker，但仅 1,653 个有 quality 标签。我们训练 GBDT 回归器，用**行为特征预测 quality**：
...
输出列 `worker_quality_pred`（0-1 归一化）。可通过 `--quality_mode predict|median` 切换。
...
| 列名 | 类型 | 含义 |
|------|------|------|
| `worker_quality` | float [0,1] | Worker 历史质量评分（原始 0-100 归一化，-1 的用中位数填充） |
| `worker_quality_raw` | float [0,100] | Worker 原始质量评分（未归一化，-1 的用中位数填充） |
...
```json
{"n_train":975195, "n_val":208971, "n_test":208971, "n_workers":1807, "n_projects":5131}
```

---

## 5. 你的任务：实现 RL 环境 (`env.py`)
...
├── sample_read_data.py        # 助教给出的参考代码（已修正 header bug）
├── worker_quality.csv
├── project_list.csv
├── project/                   # 原始项目文件
└── entry/                     # 原始 entry 文件
